  ...
  return (
    <div className="flex flex-col gap-6 px-5 pt-[max(env(safe-area-inset-top),1rem)] pb-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <StatCard icon={DollarSign} label="Revenue" value={formatPrice(summary.revenueThisWeek)} />
        <StatCard icon={CalendarCheck} label="Bookings" value={String(summary.bookingsThisWeek)} />
        <StatCard icon={UserPlus} label="New clients" value={String(summary.newClientsThisWeek)} />
        <StatCard icon={Armchair} label="Chair use" value={`${summary.chairUtilization}%`} />
        <StatCard icon={Flame} label="Deals filled" value={String(summary.hotDealsFilled)} accent />
        <StatCard icon={Star} label="Avg rating" value={summary.avgRating.toFixed(1)} />
      </div>

      <div className="rounded-2xl border border-border bg-card p-4">
        <p className="mb-3 font-semibold">Revenue by day</p>
      ...
      <div className="rounded-2xl border border-border bg-card p-4">
        <p className="mb-3 font-semibold">Bookings by day</p>
