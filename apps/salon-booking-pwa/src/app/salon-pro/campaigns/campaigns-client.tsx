...
import { Salon, ShopProduct } from "@/lib/types";
...
export function CampaignsClient({ salon, shopProducts }: { salon: Salon; shopProducts: ShopProduct[] }) {
  const initialDeals = getDealsForSalon(salon.id);
  ...
  return (
    <div className="flex flex-col gap-6">
      <BusinessHeader salon={salon} title="Campanii" subtitle="Oferte, giveaway și stoc produse" />
          ...
          onClick={() =>
            setActiveDeals((prev) => [
              ...prev,
              {
                ...
                serviceId: salon.services[0]?.id ?? "svc-1",
                seatsLeft: 3,
              },
            ])
          }
        ...
        <div className="flex flex-col rounded-2xl border border-border bg-card px-4">
          {shopProducts.map((p, i) => (
            <div key={p.id} className="flex items-center justify-between border-b border-border/70 py-3 last:border-0">
              ...
              <Badge variant={STOCK[i % STOCK.length] < 5 ? "destructive" : "secondary"}>{STOCK[i % STOCK.length]} buc.</Badge>
            </div>
          ))}
        </div>
      </div>
