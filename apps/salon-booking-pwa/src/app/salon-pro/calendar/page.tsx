...
export default function SalonProCalendarPage() {
  const appointments = useAppStore((s) => s.appointments);
  const today = new Intl.DateTimeFormat("ro-RO", { weekday: "long", month: "long", day: "numeric" }).format(new Date());

  const todays = appointments.filter((a) => a.salonId === myBusinessSalon.id && isToday(a.startIso));
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <DayTimeline barbers={myBarbers} appointments={todays} />
    </div>
  );
}
