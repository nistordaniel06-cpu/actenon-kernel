...
export function DashboardClient({ salon }: { salon: Salon }) {
  const appointments = useAppStore((s) => s.appointments);
  const todays = appointments.filter(
    (a) => a.salonId === salon.id && isToday(a.startIso) && a.status !== "anulat",
  );
  const noShows = appointments.filter((a) => a.salonId === salon.id && a.status === "no-show").length;
  ...
  return (
    <div className="flex flex-col gap-5">
      <BusinessHeader salon={salon} title="Dashboard" subtitle={`${salon.name} · ${today}`} />
