...
const filters: { key: FilterKey; label: string }[] = [
  { key: "all", label: "Toate" },
  { key: "now", label: "Liberi acum" },
  { key: "deals", label: "Oferte" },
  { key: "barbershop", label: "Frizerii" },
  { key: "salon", label: "Saloane" },
];

export function DiscoverClient({ salons }: { salons: Salon[] }) {
    ...
    <div className="flex h-[calc(100dvh-6rem)] flex-col">
      <div className="z-10 flex flex-col gap-3 bg-background pt-[max(env(safe-area-inset-top),1rem)] pb-3">
        <div className="flex items-center gap-3 px-5">
          <Link href="/home" className="flex size-9 items-center justify-center rounded-full bg-surface-2">
            <ArrowLeft className="size-4" />
          </Link>
          <h1 className="text-lg font-semibold">Descoperă</h1>
        </div>
          ...
          <div className="no-scrollbar h-full overflow-y-auto px-3 pb-6">
            {filtered.length === 0 ? (
              <p className="px-2 py-10 text-center text-sm text-muted-foreground">
                Niciun rezultat pentru acest filtru.
              </p>
            ) : (
      ...
      <Sheet open={listOpen} onOpenChange={setListOpen}>
        <SheetContent side="bottom" className="max-h-[80vh]">
          <SheetHeader>
            <SheetTitle>{filtered.length} locații aproape de tine</SheetTitle>
          </SheetHeader>
